...
class CodeInstance(TextInstance):
    """
    CodeInstance contains a labeled pair of sentences and one label.

    You could have the label represent whatever you want, in this repo the
    label indicates similarity of the two code snippets. 0 indicates that
    they are not duplicates, 1 indicates that they are.
    ...
    def read_from_line(cls, line):
        """
        Given a string line from the dataset, construct an CodeInstance from it.

        Parameters
        ----------
        line: str
            The line from the dataset from which to construct an CodeInstance
...
class IndexedCodeInstance(IndexedInstance):
    """
    ...
    def __init__(self, first_sentence_indices, second_sentence_indices, label):
        super(IndexedCodeInstance, self).__init__(label)
    ...
    def empty_instance(cls):
        return IndexedCodeInstance([], [], label=None)
    ...
    def __eq__(self, other):
        ...
        Two IndexedCodeInstance objects are equal when they have the same
        sentence lengths and the same word indices for each sentence.

        Parameters
        ----------
        other: IndexedCodeInstance
            The IndexedCodeInstance this instance is being compared against.
